...
import { mQuestionValues, mLValues, mFValues, mKValues, mHsValues, mDValues, mHyValues, mPdValues, mMfValues, mPaValues, mPtValues, mScValues, mMaValues, mSiValues } from "../values/ManValues";
...
const handleNameChange = (e) => {
  ...
  const getUpdatedQuestionValue = (parsedValue) => {
    return mQuestionValues[parsedValue] || 0;
  };
  
  const getUpdatedLValue = (parsedValue) => {
    return mLValues[parsedValue] || 0;
  };
  
  const getUpdatedFValue = (parsedValue) => {
    return mFValues[parsedValue] || 0;
  };
  
  const getUpdatedKValue = (parsedValue) => {
    return mKValues[parsedValue] || 0;
  };
  
  const mapUpdatedHsToGraphValue = (updatedHsValue) => {
    return mHsValues[updatedHsValue] || 0;
  };
  ...
  const getUpdatedDValue = (parsedValue) => {
    return mDValues[parsedValue] || 0;
  };
  
  const getUpdatedHyValue = (parsedValue) => {
    return mHyValues[parsedValue] || parsedValue;
  };
  ...
  const getUpdatedMfValue = (parsedValue) => {
    return mMfValues[parsedValue] || parsedValue;
  };
  
  const getUpdatedPaValue = (parsedValue) => {
    return mPaValues[parsedValue] || parsedValue;
  };
